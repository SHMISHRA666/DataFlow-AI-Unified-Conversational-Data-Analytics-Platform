...
class IntelligenceLayer:
    ...
    async def _generate_artifacts(self, recommendations: Dict[str, Any], 
        ...
        if has_visualizations:
            ...
            if chart_execution_result["success"]:
                ...
                # Update visualization entries with file paths and plotly links
                ...
                for viz in enhanced_output.get("generated_visualizations", []):
                    ...
                    for chart in executed_charts:
                        if chart.get("chart_id") == viz_id:
                            # Handle new chart structure with files dict
                            files_dict = chart.get("files", {})
                            viz["executed_files"] = {
                                "png_path": files_dict.get("png", {}).get("path") if "png" in files_dict else None,
                                "svg_path": files_dict.get("svg", {}).get("path") if "svg" in files_dict else None,
                                "html_path": files_dict.get("html", {}).get("path") if "html" in files_dict else None,
                                "file_sizes": {
                                    fmt: file_info.get("size_bytes", 0) 
                                    for fmt, file_info in files_dict.items()
                                }
                            }
                            break
                ...
                # Note: Individual chart plotly links removed - complete dashboard link will be generated by export agent
    ...
    async def _create_narratives(self, recommendations: Dict[str, Any],
        ...
        input_data = {
            ...
            # Provide chart execution results for plotly links and file paths
            "chart_execution_results": generated_artifacts.get("chart_execution", {}),
    ...
    def _persist_outputs(self, recommendations: Dict[str, Any],
        ...
        # 2.5) Write resolved_insights.json (per-chart insights for easy access)
        try:
            resolved_insights = narratives.get("per_chart_insights", []) if narratives else []
            resolved_path = output_dir / "resolved_insights.json"
            with resolved_path.open("w", encoding="utf-8") as f:
                json.dump(resolved_insights, f, indent=2, ensure_ascii=False)
        except Exception as e:
            log_error(f"Error writing resolved_insights.json: {e}")
