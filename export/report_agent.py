...
def write_outputs(
    ...
    # Do NOT copy assets to the report root; instead, reference subfolders (html/png/svg)
    # Normalize references to be relative to out_dir so the HTML uses subfolder paths.
    for it in report.insights:
        if it.references.html_path:
            it.references.html_path = str(Path("html") / Path(it.references.html_path).name)
        if it.references.png_path:
            it.references.png_path = str(Path("png") / Path(it.references.png_path).name)
        if it.references.svg_path:
